...
if os.path.exists('side_quests/house_parameters/house_parameters_by_month.csv'):
    os.remove('side_quests/house_parameters/house_parameters_by_month.csv')

for house_alias in ['beech', 'fir', 'oak', 'maple', 'elm']:
    for year in [2024, 2025]:
        for month in [9,10,11,12,1,2,3,4,5]:

            month_print = f"0{month}/{year}" if month < 9 else f"{month}/{year}"
            print(f"\n{house_alias} - {month_print}:")

            # Outputs from backoffice electricity use CSV
            df = pd.read_csv(f'side_quests/data/energy_data_{house_alias}.csv')

            # Get time in unix seconds to merge with weather data
            df['hour_start_ms'] = df['hour_start_ms'] - 4*3600*1000 - 1000
            df['hour_start_dt'] = pd.to_datetime(df['hour_start_ms'], unit='ms', utc=False)
            ...
            df['year'] = [x.year for x in dates_temp]
            ...
            weather_and_prices_df = pd.read_csv('side_quests/data/simulation_data.csv')
            ...
            csv_file = 'side_quests/house_parameters/house_parameters_by_month.csv'
...
# Plot for each house
if not os.path.exists('side_quests/house_parameters/house_parameters_by_month.csv'):
    print("\nNo parameters have been generated")

else:
    params_df = pd.read_csv('side_quests/house_parameters/house_parameters_by_month.csv')
